...
export function buildEndpointAgentInstructions({
  ...
  return [
    ...
    "- When the final bundle validates and any requested promotion step is complete, call finalize_validated_bundle. Do not continue probing or editing after final validation.",
    ...
    "- Return the required AgentRunSummary object only after files are written and validation has been attempted.",
    "- Prefer calling finalize_validated_bundle to complete the run; the runner will stop there.",
...
export function buildEndpointAgentTask({ slug, outRoot, currentDate, promote }: BuildTaskInput): string {
  return [
    ...
    `- write_artifact_file/list_output_files/promote_semantic_bundle outRoot: "${outRoot}"`,
